...
public class Controlador {
	...
	//Instancia de User para inicializar los usuarios en un List.
	...
	//Método para emcriptar un mensaje y añadirlo al listado.
	public String encriptar(String mensaje) {
		mensajeCifrado = encriptador.encriptar(mensaje);
		return mensajeCifrado;
		
	}
	//Método para add un mensaje al array, relacioando con DAO
		...
		//Método para llamar al metodo listar de DAO
	...
	public String getMensaje(int posicion) {
		...
		for(MensajeEncriptado lista : listado) {
			...
			//Si encuentra coincidencia retorno inmediatamente y ya luego fuera iría el caso de no coincidir
	...
	public boolean confirmUser(String user, String password) throws NoSuchAlgorithmException {
		
		//Si hay fallo, es por el Base64??? y el thorws me lo pide solo
		String contraseña = HashConversor.conversor(password);
		
		for(User lista : userList) {
			if(user.equals(lista.getName()) && contraseña.equals(lista.getPassword())){
				return true;
			}
		}
		return false;
